import sys
import numpy as np
import eval_metrics_2021 as em
import matplotlib.pyplot as plt
import os

def evaluate_tDCF_asvspoof21(cm_score_file, asv_score_file, legacy, phase):
    ...
    asv_key_file = os.path.join(asv_score_file, 'ASV/trial_metadata.txt')
    asv_scr_file = os.path.join(asv_score_file, 'ASV/ASVTorch_Kaldi/score.txt')
    asv_key_data = np.genfromtxt(asv_key_file, dtype=str)
    asv_scr_data = np.genfromtxt(asv_scr_file, dtype=str)

    asv_phase = asv_key_data[:, 7]
    asv_keys = asv_key_data[:, 5]
    asv_scores = asv_scr_data[:, 2].astype(np.float)
    asv_keys = asv_keys[asv_phase == phase]
    asv_scores = asv_scores[asv_phase == phase]
    ...
    cm_keys = cm_keys[cm_sources == phase]
    cm_scores = cm_scores[cm_sources == phase]
